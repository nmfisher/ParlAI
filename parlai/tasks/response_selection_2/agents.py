...
class PersonaAgnosticTeacher(FbDialogTeacher):
    def act(self):
        action = super().act();
        text = ""
        for line in action["text"].split("\n"):
            if not line.startswith("your persona:"):
                text += line
        if text.startswith("__SILENCE__"):
            return self.act()
        action.force_set("text", text)
        return action
        
class BothTeacher(PersonaAgnosticTeacher):
...
class NoneTeacher(PersonaAgnosticTeacher):
    def __init__(self, opt, shared=None):
        opt = copy.deepcopy(opt)
        try:
            cands = opt['task'].split(":")[2]
...
class SelfOriginalTeacher(PersonaAgnosticTeacher):
    def __init__(self, opt, shared=None):
        opt = copy.deepcopy(opt)
        try:
            cands = opt['task'].split(":")[2]
...
class SelfRevisedTeacher(PersonaAgnosticTeacher):
    def __init__(self, opt, shared=None):
        opt = copy.deepcopy(opt)
        try:
            cands = opt['task'].split(":")[2]
